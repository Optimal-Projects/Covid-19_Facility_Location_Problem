...
def covid_main():
    ...
    for country in countries:
        for facility in facilities:
            distance.update({(country,facility): math.dist(eval(coordinate_country_dict[country]), eval(coordinate_facility_dict[facility]))})
    print(distance)


    temporary_cost = 500000
    bigM = random.randrange(2000000)
    dCost = 5                    #cost of travelling 10 miles
    ...
    for c in countries:
        for f in facilities:
            Number_of_patients.update({(c,f): pl.LpVariable("Num_" + str(c) + "_" + str(f), cat="Continuous",lowBound = 0)})
    ...
    for c in countries:
        for f in facilities:
            prob += Number_of_patients[c,f] >= 0

    for t in temp_facility:
        prob += extra_capacity[t] >= 0
